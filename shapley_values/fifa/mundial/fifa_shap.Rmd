...
Boston <- read.csv("/Users/dfontenla/Maestria/2022C2/EEA/practica/repo/EEA-2022/TP2/fifa/FIFA 2018 Statistics.csv", encoding = "UTF-8") %>% mutate(id = 1:nrow(.)) 
glimpse(Boston)

x_var <- setdiff( colnames( Boston ), c( "Man.of.the.Match" ) )
x_var <- c("Goal.Scored", "Ball.Possession..", "Attempts", "On.Target", "Blocked", "Off.Target", "Corners", "Offsides", "Free.Kicks ", "Saves", "Pass.Accuracy..", 
"Passes", "Distance.Covered..Kms.", "Fouls.Committed", "Yellow.Card", "Yellow...Red", "Red", "X1st.Goal", "Goals.in.PSO", "Own.goals")

y_var <- "Man.of.the.Match"

x_train <- as.matrix(Boston[, x_var])
y_train <- Boston[, y_var]

x_test <- as.matrix(Boston[, x_var])
x_test
```
